...
export function DashboardHeader({
  ...
  const subtitleParts = [visibleGoalsCount === 1 ? t("dashboard.metaVisibleOne") : t("dashboard.metaVisibleMany", {
    n: visibleGoalsCount
  }), t("dashboard.percentDone", {
    n: overallPercent
  })];
  return <header className={cn("space-y-6", className)}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        duration: 0.4,
        ease: [0.22, 1, 0.36, 1]
      }} className={cn("flex max-w-sm items-start gap-3 rounded-2xl border px-4 py-3", dayComplete ? "border-emerald-500/25 bg-emerald-500/[0.07] text-emerald-100/95" : "border-white/[0.08] bg-white/[0.03] text-zinc-300")}>
          <Sparkle className={cn("mt-0.5 h-5 w-5 shrink-0", dayComplete ? "text-emerald-400" : "text-violet-400")} weight="fill" />
