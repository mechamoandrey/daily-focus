    ...
    <AppShell>
      <main className="flex-1 px-4 py-8 sm:px-8 sm:py-12 lg:px-12">
        <div className="mx-auto max-w-5xl space-y-10">
            {syncError ? (
              <p className="rounded-xl border border-amber-500/25 bg-amber-500/[0.06] px-4 py-2 text-center text-sm text-amber-100/95">
                {translateSyncError(syncError, t)}
              </p>
            ) : null}
            <DashboardHeader
              dayComplete={dayComplete}
              dateLabel={formatYMDLongLocalized(dayYmd, locale)}
              visibleGoalsCount={stats.goalsTotal}
              overallPercent={overallPercent}
            />

            <DailyProgress
              percent={overallPercent}
              subDone={stats.subDone}
              subTotal={stats.subTotal}
              goalsComplete={stats.goalsComplete}
              goalsTotal={stats.goalsTotal}
              dayComplete={dayComplete}
            />

            <StreakCard
              archivedStreak={state.streak}
              ...
              <div className="grid gap-5 lg:grid-cols-2">
                <AnimatePresence mode="popLayout">
                  ...
                  {linkedinToday ? (
                    <LinkedInFridayCard
                      key="linkedin-friday"
                      goal={state.linkedinFriday}
                      index={dashboardGoals.length}
                      onToggle={toggleSubtask}
                    />
                  ) : null}
                </AnimatePresence>
