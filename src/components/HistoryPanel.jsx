...
export function HistoryPanel({ entries, className }) {
  ...
  return (
    <motion.section
      ...
      className={cn(
        "rounded-2xl border border-white/[0.06] bg-zinc-950/50 p-5 sm:p-6",
        className
      )}
    >
      ...
      {list.length === 0 ? (
        <p className="text-sm text-zinc-500">{t("panel.empty")}</p>
      ) : (
        <ul className="max-h-[min(28rem,55vh)] divide-y divide-white/[0.05] overflow-y-auto pr-1 [scrollbar-color:rgba(255,255,255,0.12)_transparent] [scrollbar-width:thin]">
          {list.map((row) => (
            <li
              key={row.date}
