...
export function LinkedInFridayCard({ goal, onToggle, index }) {
  const { t } = useLocale();
  ...
  return (
    <motion.article
      ...
      className={cn(
        "relative overflow-hidden rounded-2xl border border-sky-500/20 bg-gradient-to-br from-sky-950/50 to-zinc-950/90 shadow-[0_24px_80px_-32px_rgba(0,0,0,0.85)] ring-1 ring-sky-500/10 transition-opacity duration-200 hover:opacity-[0.93] lg:col-span-2"
      )}
    >
      ...
      <div className="relative space-y-5 p-5 sm:p-6">
        ...
        <div className="space-y-2">
          ...
          <div className="relative h-2 overflow-hidden rounded-full bg-zinc-800/90 ring-1 ring-white/[0.05]">
            <motion.div
              className="h-full rounded-full bg-gradient-to-r from-sky-400 to-[#0A66C2]"
              initial={{ width: 0 }}
              animate={{ width: `${pct}%` }}
        ...
        <div className="space-y-1.5">
          {display.subtasks.map((s) => (
            <SubtaskItem
              key={s.id}
              goalId={LINKEDIN_FRIDAY_META.id}
              subtask={s}
              onToggle={onToggle}
            />
          ))}
