...
/**
 * @param {{ archivedStreak: number, todayBonus: boolean, displayStreak: number }} props
 */
export function StreakCard({ archivedStreak, todayBonus, displayStreak, className }) {
  const { t } = useLocale();
  const subLine =
    displayStreak === 0
      ? t("streak.sub0")
      : displayStreak === 1
        ? t("streak.sub1")
        : t("streak.subN");
  const footer = todayBonus
    ? t("streak.todayCounts")
    : archivedStreak > 0
      ? t("streak.extend")
      : t("streak.start");
  ...
  return (
    <motion.div
      ...
      className={cn(
        "rounded-2xl border border-orange-500/20 bg-gradient-to-br from-orange-500/[0.09] to-zinc-950/40 p-5 ring-1 ring-orange-500/10",
        className
      )}
    >
