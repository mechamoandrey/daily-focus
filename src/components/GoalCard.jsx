...
export function GoalCard({ goal, index, onToggle }) {
  const { t } = useLocale();
  ...
  return (
    <motion.article
      ...
      className={cn(
        "relative overflow-hidden rounded-2xl border border-white/[0.07] bg-zinc-900/40 shadow-[0_24px_80px_-32px_rgba(0,0,0,0.85)] ring-1 ring-white/[0.04] transition-opacity duration-200 hover:opacity-[0.93]"
      )}
    >
      <div
        className={cn(
      ...
      <div className="relative space-y-5 p-5 sm:p-6">
        <div className="space-y-2">
          {goal.category ? (
            <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-violet-400/90">
              {goal.category}
            </p>
          ) : null}
