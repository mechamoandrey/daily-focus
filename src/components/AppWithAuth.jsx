...
function AuthScreen() {
  ...
  return <div className="relative flex min-h-screen flex-col items-center justify-center bg-zinc-950 px-4">
      ...
      ease: [0.22, 1, 0.36, 1]
        ...
        {authError || err ? <p className="mb-4 rounded-xl border border-rose-500/25 bg-rose-500/[0.08] px-3 py-2 text-sm text-rose-100/95">
            {authError || (typeof err === "string" && err.startsWith("auth.") ? t(err) : err)}
          </p> : null}
        <motion.button type="button" disabled={busy || !supabase} onClick={handleGoogle} whileHover={{
        scale: busy ? 1 : 1.01
      }} whileTap={{
        scale: busy ? 1 : 0.99
...
function AuthLoading() {
  ...
  return <div className="flex min-h-screen flex-col items-center justify-center bg-zinc-950 px-4">
