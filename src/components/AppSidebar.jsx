...
import { ClockCounterClockwise, Package, SignOut, Target } from "@phosphor-icons/react";
...
export function AppSidebar({
  ...
  return <motion.aside initial={{
    ...
    duration: 0.45,
    ease: [0.22, 1, 0.36, 1]
  }} className={cn("hidden h-screen w-56 shrink-0 flex-col border-r border-white/[0.06] bg-zinc-950/40 px-5 py-8 lg:sticky lg:top-0 lg:flex", className)}>
      ...
      <div className="mt-10 space-y-1 text-sm">
        ...
        <nav className="space-y-1">
          ...
          return <Link key={href} href={href} className={cn("flex cursor-pointer items-center gap-2 rounded-xl px-3 py-2 transition-colors duration-200", active ? "bg-white/[0.06] text-zinc-100 ring-1 ring-white/[0.06]" : "text-zinc-400 hover:bg-white/[0.04] hover:text-zinc-200")}>
      ...
      {user ? <div className="mt-auto border-t border-white/[0.06] pt-6">
          <p className="truncate px-2 text-[11px] text-zinc-500" title={user.email ?? ""}>
            {user.email ?? t("auth.account")}
          </p>
          <button type="button" onClick={handleSignOut} className="mt-2 flex w-full cursor-pointer items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-zinc-400 transition hover:bg-white/[0.04] hover:text-zinc-200">
