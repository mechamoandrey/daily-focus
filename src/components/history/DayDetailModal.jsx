...
import { buildDayNarrative, dayPerformanceLabelKey } from "@/lib/historyHeatmapDay";
...
export function DayDetailModal({
  ...
  const dayClass = pct != null ? t(dayPerformanceLabelKey(pct)) : "";
  return createPortal(<AnimatePresence mode="wait">
      {entry ? <motion.div key={entry.date} role="dialog" aria-modal="true" aria-labelledby="heatmap-day-modal-title" className="fixed inset-0 z-[80] flex items-end justify-center p-4 sm:items-center" initial={{
      opacity: 0
    }} animate={{
      opacity: 1
    }} exit={{
      ...
      duration: 0.2
        ...
        ease: [0.22, 1, 0.36, 1]
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <p id="heatmap-day-modal-title" className="text-[11px] font-medium uppercase tracking-[0.18em] text-zinc-500">
                  {t("dayModal.title")}
                </p>
