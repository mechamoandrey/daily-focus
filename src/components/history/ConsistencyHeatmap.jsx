...
import {
  getGoalPercentInEntry,
  getHeatmapDaySummary,
...
/**
 * @param {{ history: object[], variants?: object, goalColorId?: string | null }} props
 * `history` deve ser a janela já filtrada por período (ex.: últimos 90 dias).
 */
export function ConsistencyHeatmap({ history, variants, goalColorId }) {
  const { t } = useLocale();
  ...
  const tooltipSummary = useMemo(
    () =>
      hover ? getHeatmapDaySummary(hover.entry, { goalId: goalColorId }) : null,
    [hover, goalColorId]
  );
  ...
  const handlePointerEnter = useCallback((entry, el) => {
    if (!el) return;
    setHover({ entry, rect: el.getBoundingClientRect() });
  }, []);
  ...
  return (
    <motion.div
      variants={variants}
      className="rounded-2xl border border-white/[0.06] bg-zinc-950/40 p-5 sm:p-6"
    >
          ...
          if (entry) {
            return (
              <motion.button
                ...
                type="button"
                {...commonHandlers}
                onFocus={(e) => handlePointerEnter(entry, e.currentTarget)}
                ...
                aria-label={label}
                className={cn(
                  "aspect-square rounded-md ring-1 transition duration-200",
                  "outline-none focus-visible:ring-2 focus-visible:ring-violet-400/60",
                  "cursor-pointer hover:ring-white/25 hover:brightness-110",
                  cellVisual(entry, goalColorId)
                )}
              />
            );
          }
