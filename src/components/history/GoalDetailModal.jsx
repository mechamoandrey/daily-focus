 ...
 * }} props
 */
export function GoalDetailModal({ goalRow, history, onClose }) {
  const { t, locale } = useLocale();
  const displayTitle = goalRow
    ? getLocalizedGoalTitle(String(goalRow.id), goalRow.title, t)
    : "";
  ...
  const recent = useMemo(() => {
    ...
    const out = [];
    for (const e of desc) {
      const g = getDetailGoals(e).find(
        (x) => String(x.id) === String(goalRow.id)
      );
      if (g) out.push({ date: e.date, g });
            ...
            <div className="mt-4 border-t border-white/[0.06] pt-4">
              ...
              <ul className="mt-2 space-y-2 text-sm">
                {recent.map(({ date, g }) => (
                  <li
                    key={date}
                    className="flex items-center justify-between gap-2 rounded-lg bg-zinc-900/50 px-3 py-2 text-zinc-300"
                  >
