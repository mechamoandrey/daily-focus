...
import { ALL_WEEKDAY_KEYS, normalizeGoalVisibleDays, normalizeLinkedinVisibleDays, weekdayKeyFromYmd } from "@/lib/visibleDays";
...
export function normalizeSubtask(s, fallbackId) {
  ...
  const id = typeof raw.id === "string" && raw.id.length > 0 ? raw.id : fallbackId || crypto.randomUUID();
  const done = Boolean("completed" in raw ? raw.completed : "done" in raw ? raw.done : false);
  const completed = done;
  const label = typeof raw.label === "string" && raw.label.trim().length > 0 ? raw.label.trim() : "Subtarefa";
  const createdAt = typeof raw.createdAt === "string" ? raw.createdAt : ISO_NOW();
...
export function normalizeSystemGoal(template, stored, index) {
  ...
  const subMap = new Map((Array.isArray(prev.subtasks) ? prev.subtasks : []).map(s => [s.id, s]));
  ...
  if (prevSubs.length === 0) {
    ...
    subtasks = prevSubs.filter(s => s && typeof s === "object").map(s => {
      const old = subMap.get(s.id);
      const tid = templateById.get(s.id);
      const label = typeof s.label === "string" && s.label.trim().length > 0 ? s.label.trim() : tid && typeof tid.label === "string" ? tid.label : "Subtarefa";
      return normalizeSubtask({
        ...(old && typeof old === "object" ? old : {}),
        ...s,
        id: s.id,
  ...
  const now = ISO_NOW();
  const title = typeof prev.title === "string" && prev.title.trim().length > 0 ? prev.title.trim() : template.title;
  const description = typeof prev.description === "string" ? prev.description : template.description;
  return {
    id: template.id,
    userId: prev.userId ?? null,
    title,
    description,
    category: typeof prev.category === "string" && prev.category.trim().length > 0 ? prev.category.trim() : null,
    subtasks,
...
export function normalizeUserGoal(raw) {
  ...
  const id = typeof g.id === "string" && g.id.length > 0 ? g.id : crypto.randomUUID();
  const title = typeof g.title === "string" && g.title.trim().length > 0 ? g.title.trim() : "Nova meta";
  const description = typeof g.description === "string" ? g.description : "";
  const category = typeof g.category === "string" && g.category.trim().length > 0 ? g.category.trim() : null;
...
export function mergeGoalsWithTemplates(storedGoals) {
  ...
  const system = INITIAL_GOALS.map((template, idx) => normalizeSystemGoal(template, byId.get(template.id), idx));
