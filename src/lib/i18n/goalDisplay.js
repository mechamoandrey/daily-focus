...
import { INITIAL_LINKEDIN_SUBTASKS, LINKEDIN_FRIDAY_META } from "@/data/linkedinFriday";
...
export function getLocalizedGoalTitle(goalId, storedTitle, t) {
  if (goalId === LINKEDIN_GOAL_ID) {
    if (stripMatch(storedTitle, LINKEDIN_FRIDAY_META.title)) return t("linkedin.meta.title");
    return storedTitle;
  }
...
export function getLocalizedGoalDescription(goalId, storedDescription, t) {
  if (goalId === LINKEDIN_GOAL_ID) {
    if (stripMatch(storedDescription, LINKEDIN_FRIDAY_META.description)) return t("linkedin.meta.description");
    return storedDescription ?? "";
  }
  const tpl = TEMPLATE_BY_ID.get(goalId);
  if (!tpl || !SYSTEM_GOAL_IDS.has(goalId)) return storedDescription ?? "";
  if (stripMatch(storedDescription, tpl.description)) return t(`systemGoal.${goalId}.description`);
  return storedDescription ?? "";
}
export function getLocalizedSubtaskLabel(goalId, subtaskId, storedLabel, t) {
  ...
  if (goalId === LINKEDIN_GOAL_ID) {
    ...
    if (li && stripMatch(storedLabel, li.label)) return t(`linkedin.sub.${subtaskId}.label`);
    return storedLabel;
  }
  ...
  if (sub && subtaskId && stripMatch(storedLabel, sub.label)) return t(`systemGoal.${goalId}.sub.${subtaskId}`);
  return storedLabel;
}
export function localizeSubtaskLabel(goalId, storedLabel, t) {
  ...
  if (goalId === LINKEDIN_GOAL_ID) {
    for (const s of INITIAL_LINKEDIN_SUBTASKS) {
      if (stripMatch(storedLabel, s.label)) return t(`linkedin.sub.${s.id}.label`);
    }
    return storedLabel;
  }
  const tplGoal = TEMPLATE_BY_ID.get(goalId);
  ...
  for (const s of tplGoal.subtasks) {
    if (stripMatch(storedLabel, s.label)) return t(`systemGoal.${goalId}.sub.${s.id}`);
  }
  return storedLabel;
}
export function getLocalizedSubtaskHint(goalId, subtaskId, storedHint, t) {
  ...
  if (goalId === LINKEDIN_GOAL_ID) {
    const li = INITIAL_LINKEDIN_SUBTASKS.find(s => s.id === subtaskId);
    if (li?.hint && stripMatch(storedHint, li.hint)) return t(`linkedin.sub.${subtaskId}.hint`);
  }
  return storedHint;
}
export function resolveGoalDisplay(goal, t) {
  const gid = goal && typeof goal === "object" && typeof goal.id === "string" ? goal.id : "";
  if (!gid) {
    return {
      title: String(goal?.title ?? ""),
      description: goal?.description ?? "",
      subtasks: Array.isArray(goal?.subtasks) ? goal.subtasks.map(s => ({
        ...s
      })) : []
    };
  }
