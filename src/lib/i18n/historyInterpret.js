/** @param {string} kind @param {object} snapshot @param {(k: string) => string} t */
  ...
  switch (kind) {
    ...
    case "streak":
      return snapshot.currentStreak > 0
        ? t("history.interpret.streak.on")
        : t("history.interpret.streak.off");
    case "bestStreak":
      return snapshot.bestStreak > 0
        ? t("history.interpret.bestStreak.yes")
        : t("history.interpret.bestStreak.no");
    case "fullDayRate":
    ...
    case "trend": {
      ...
      if (trend.direction === "insufficient")
        return t("history.interpret.trend.insufficient");
