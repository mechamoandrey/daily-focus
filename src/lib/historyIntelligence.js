import { computeAnalyticsSnapshot, getDayProgress, getDetailGoals, getGoalStats, getSubtaskStats, getTrend, getTrendWindow, getWeekdayStats, goalIsComplete100, sortedAsc, sortedDesc } from "@/lib/analytics";
...
export function computeGoalDeepStats(history) {
  ...
  for (const id of out.keys()) {
    ...
    for (const e of descAll) {
      const g = getDetailGoals(e).find(x => String(x.id) === id);
      if (!g) continue;
      if (goalIsComplete100(g)) run += 1;else break;
    }
...
export function computeGoalRankings(deep) {
  ...
  return {
    ...
    mostNeglected: [...eligible].sort((a, b) => b.failureRate - a.failureRate)[0],
    bestAvg: byAvg[0],
